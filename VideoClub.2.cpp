...
public:
	ChildrensMovie(string t) : Movie(t) {}
	int get_price(int days_rented) { 
		int price = 1500;
		if (days_rented > 3)
...
public:
	NewReleaseMovie(string t) : Movie(t) {}
	int get_price(int days_rented) { 
...
int main() {
	...
	Catalog cat;
	cat.add_movie(new RegularMovie("Ezhdehaa-ye Vahshi"));
	cat.add_movie(new RegularMovie("Faasele-haa Dar Ofogh"));
	cat.add_movie(new ChildrensMovie("Sara Dar Mazra'eh"));
	cat.add_movie(new ChildrensMovie("Jooje-ye Saboor"));
	cat.add_movie(new NewReleaseMovie("Arre-ye Kond"));
	cat.add_movie(new NewReleaseMovie("Nabard-e Zangbaar"));
